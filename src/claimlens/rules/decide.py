...
def _issue_compatible(claimed_issue: str, observed_issue: str) -> bool:
    ...
    if claimed_issue == observed_issue:
        return True
    return any(claimed_issue in fam and observed_issue in fam for fam in _ISSUE_FAMILIES)


def _damage_on_claimed_part(img: ImageObservation, claimed: list[str], claimed_issue: str) -> bool:
...
def decide(claim: PreparedClaim, observation: VLMObservation | None) -> OutputRow:
    ...
    assessable = any(
        im.claimed_part_visible or im.damage_present or not im.matches_claim_object for im in images
    )
...
def _contradict_or_nei(
    ...
    wrong_object_imgs = [im for im in images if not im.matches_claim_object and im.object_in_image]
