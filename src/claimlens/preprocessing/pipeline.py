...
def prepare_claims(
    ...
    for claim in claims:
        images = [encode_image(rel, dataset_dir, config.image) for rel in claim.image_path_list]
        prepared.append(
            PreparedClaim(
                claim_input=claim,
...
def prepare_dataset(
    ...
    requirements = load_evidence_requirements(cfg.resolve(cfg.paths.evidence_requirements_csv))
