...
Loads ``configs/default.yaml`` into typed settings and resolves all paths
against the repo root so the pipeline runs identically regardless of the
current working directory. API keys are never stored here - only the *name* of
the env var to read at call time.
"""
...
# repo_root / src / claimlens / config.py  ->  parents[2] == repo root
...
class Config(BaseModel):
    ...
    def active_model_config(self) -> ModelConfig:
        if self.active_model not in self.models:
            raise KeyError(
                f"active_model '{self.active_model}' not found in models: {sorted(self.models)}"
            )
        return self.models[self.active_model]


def load_config(path: str | Path | None = None) -> Config:
    """Load and validate a config file (defaults to ``configs/default.yaml``)."""
