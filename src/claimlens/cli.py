...
Installed as the ``claimlens`` console script. Subcommands, cheapest first:

    claimlens preprocess --set test    # load + encode a dataset, print a summary
    claimlens dryrun --set sample      # check the model-call wiring offline
    claimlens validate                 # score the rule layer on cached observations
    claimlens verify                   # validate a predictions CSV against its claims
    claimlens smoke --set sample --n 2 # live end-to-end check on N claims
    claimlens run --set test           # full run -> output.csv
    claimlens evaluate                 # compare models, rewrite the evaluation report

Only ``smoke``, ``run`` and ``evaluate`` make API calls.
"""
...
def cmd_provider_dryrun(args: argparse.Namespace) -> int:
    """Offline check of the provider wiring: build messages + cache key and
    ...
    for p in prepared:
        ...
        img_blocks = sum(1 for b in msgs[1]["content"] if b.get("type") == "image_url")
    ...
    print(
        f"  user image blocks: {sum(1 for b in msgs0[1]['content'] if b.get('type') == 'image_url')}"
    )
...
def cmd_smoke(args: argparse.Namespace) -> int:
    ...
    for p in prepared:
        ...
        print(
            f"  tokens:          prompt={res.prompt_tokens} "
...
def cmd_run(args: argparse.Namespace) -> int:
    ...
    rows = [decide(p, r.observation) for p, r in zip(prepared, results, strict=True)]
    ...
    print(
        f"calls: {new_calls} new, {cached} cached, {errors} errors | "
...
def cmd_validate(args: argparse.Namespace) -> int:
    ...
    for p, exp in zip(prepared, expected, strict=True):
...
def cmd_verify(args: argparse.Namespace) -> int:
    """Validate a predictions CSV against the claims file it answers (offline)."""
    from claimlens.verify import format_result, verify

    cfg = load_config(args.config)
    predictions = Path(args.predictions) if args.predictions else cfg.resolve(cfg.paths.output_csv)
    claims = Path(args.claims) if args.claims else cfg.resolve(cfg.paths.claims_csv)

    print(f"predictions: {predictions}")
    print(f"claims:      {claims}\n")
    result = verify(predictions, claims)
    print(format_result(result))
    return 0 if result.ok else 1


def cmd_evaluate(args: argparse.Namespace) -> int:
    """Compare model configurations on the labeled sample and rewrite the report."""
    from claimlens.evaluation import report
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="claimlens",
        description="Multi-modal damage-claim evidence review",
    )
    parser.add_argument(
        "--config",
        default=None,
        help="Path to a config file (default: configs/default.yaml)",
    )
    ...
    dr = sub.add_parser(
        "dryrun",
        help="Offline check of the provider wiring (no API call / no key needed)",
    )
    ...
    val = sub.add_parser(
        "validate", help="Score the rule layer vs labeled sample (cached only, no spend)"
    )
    val.set_defaults(func=cmd_validate)

    ver = sub.add_parser(
        "verify", help="Validate a predictions CSV against its claims file (offline)"
    )
    ver.add_argument(
        "--predictions", default=None, help="Predictions CSV (default: config paths.output_csv)"
    )
    ver.add_argument(
        "--claims",
        default=None,
        help="Claims CSV to check against (default: config paths.claims_csv)",
    )
    ver.set_defaults(func=cmd_verify)

    ev = sub.add_parser(
        "evaluate", help="Compare model configurations and rewrite the evaluation report"
    )
    report_module.add_arguments(ev)
