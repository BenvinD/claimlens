"""Model comparison and evaluation-report rendering.

Runs the pipeline over the labeled sample set for one or more model
configurations, scores the predictions against the labels, compares the runs and
writes ``docs/evaluation-report.md`` (accuracy metrics plus operational
analysis: calls, tokens, cost, latency, rate-limit posture).

Invoked as ``claimlens evaluate``.
"""
...
from claimlens.config import REPO_ROOT, load_config
from claimlens.evaluation import EvalRun, run_model
...
def _parse_specs(
    models: list[str], runs: list[str] | None, default_pv: str
) -> list[tuple[str, str]]:
...
def _render(runs: list[EvalRun], cfg, test_claims: int, test_images: int, final: str) -> str:
    L: list[str] = []
    L.append("# ClaimLens — Evaluation Report\n")
    L.append(f"_Generated: {datetime.now().isoformat(timespec='seconds')}_\n")
    L.append(
        "> Sections 1-4 are regenerated by `claimlens evaluate`. Any curated\n"
        "> analysis appended below them must be re-added after a regeneration.\n"
    )
    L.append(
    ...
    for r in runs:
        ...
        L.append(
            f"- **`{r.label}`**: "
            + ", ".join(f"`{k}`×{v}" for k, v in sorted(r.metrics.status_confusion.items()))
        )
    L.append("")
    L.append(
    ...
    for r in runs:
        if r.metrics:
            L.append(
                f"> - `{r.label}`: manual_review_required recall "
    ...
    L.append(
        f"Test-set projection ({test_claims} claims, {test_images} images), "
    ...
    for r in runs:
        ...
        price = (
            f"${mc.input_per_mtok}/{mc.output_per_mtok}" if mc.input_per_mtok is not None else "n/a"
        )
    ...
    L.append(
        "**Calls / images:** exactly one model call per claim; multiple images "
...
def add_arguments(parser: argparse.ArgumentParser) -> None:
    """Register the ``evaluate`` options on a parser or subparser."""
    parser.add_argument(
        "--models",
        nargs="+",
        default=["claude-sonnet", "claude-haiku"],
        help="Model keys from the config's `models:` table to compare",
    )
    parser.add_argument(
        "--runs",
        nargs="+",
...
def run(args: argparse.Namespace) -> int:
    """Evaluate every requested configuration and rewrite the report."""
    ...
    for name, pv in specs:
        ...
        if run.metrics:
            m = run.metrics
            print(
                f"  claim_status acc: {_fmt_pct(m.column_accuracy.get('claim_status', 0))} "
